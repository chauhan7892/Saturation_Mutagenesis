from Burrow_Wheeler_sql_v1 import *
...
def main( ):
    ...
    SAMPLE = input_file[2] ## sample name
    BATCH_SIZE = int(input_file[3]) ## set batch size


    ## Files
    wt_full_seq_file = input_file[4]
    ...
    try:
        ...
        offset = 0
        while count < 1200:#True:
            cursor.execute(f"SELECT seq_id FROM {TABLE_NAME} WHERE sample = ? LIMIT ? OFFSET ?", (SAMPLE, BATCH_SIZE, offset))
            seq_ids_tuple = cursor.fetchall()
            if not seq_ids_tuple:
                # all rows have been fetched
                break
            
            # process the batch of results
            for row in seq_ids_tuple:
                # get list from tuples
            ...
            for chunk_ids in tqdm(each_processor_chunk_ids):
